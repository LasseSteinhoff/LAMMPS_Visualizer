...
public class Master_Program_File : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (!Directory.Exists(Application.dataPath + "/format"))
    ...
    void Update()
    {
        ...
        if (!reached_end)
        {
            ...
            else if ((Input.GetKey(KeyCode.LeftShift) && Input.GetKey(KeyCode.E) || Input.GetKeyDown(KeyCode.E))) // forward in time
            {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                // if bond analysis was executed, it can present the total number of bonds in a figure
                        ...
                        if (bond_figures != null)
                            foreach (Bond bond in bonds)
                                if (ReferenceEquals(bond.bond, clickedObject))
                                    if (bond_figures.ContainsKey(bond.gameObject))
                                        info_label.text = structures[bond_figures[bond.gameObject]].Count.ToString() + " proteins involved";
                                    else if (bond_figures.ContainsKey(bond.gameObject2))
                                        info_label.text = structures[bond_figures[bond.gameObject2]].Count.ToString() + " proteins involved";
                    }
                    catch (Exception e) { info_label.text = "Not Found!"; }
                }
                else
                {
                    // presents the protein name of a clicked object
                    foreach (Protein protein in proteins)
                        if (protein.name.Equals(clickedObject.name))
    ...
    private void CreateBonds()
    {
        ...
        // apply a random recursion for a global minium tree
        if (bond_minimize)
        {
        ...
        else
        {
            ...
            {   // apply protein format
                if (format_binding_distance)
                {
                    ...
                    {
                        foreach (GameObject partner in CheckCollision(Physics.OverlapSphere(gameObject.transform.position, protein.bindingdistance.Values.Max()), gameObject))
                        {
    ...
    private void BondAnalysis(List<Bond> bonds)
    {
        ...
        same_structure_size.Clear();
        protein_combinations.Clear();
        gameObject_for_teleport.Clear();

        foreach (List<GameObject> structure in structures)
        {
            string key = string.Empty;
            ArrayList protein_combination = new ArrayList();
